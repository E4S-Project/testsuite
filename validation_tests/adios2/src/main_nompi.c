/*
 * Adapted from ADIOS2 upstream testing/install/C/main_nompi.c (Apache-2.0):
 * writes and reads back a small array to exercise real I/O, not just linking.
 */
...
{
    char bp_name[256];
    snprintf(bp_name, sizeof(bp_name), "%s.bp", argc > 0 ? argv[0] : "adios2_c_smoke_test");
    ...
    if (!rvar)
    {
        fprintf(stderr, "variable 'data' not found on read\n");
        return 1;
    }
    ...
    for (int i = 0; i < NX; ++i)
    {
        ...
        {
            fprintf(stderr, "mismatch at %d: wrote %d read %d\n", i, write_data[i], read_data[i]);
            return 1;
        }
    }

    printf("adios2 C smoke test: wrote and verified %d values\n", NX);
    return 0;
}
