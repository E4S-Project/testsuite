...
int main(int argc, char** argv) {
    ...
    if (rank < sizeof(k) / sizeof(char*)) {
        ...
        if (ret != PAPYRUSKV_OK) printf("FAILED:[%s:%d] ret[%d] rank[%d] key[%s]\n", __FILE__, __LINE__, ret, rank, k[rank]);
        printf("[%s:%d] GET:rank[%d] key[%s] value[%s] vallen[%lu]\n", __FILE__, __LINE__, rank, k[rank], val, vallen);
    }
    ...
    if (left < sizeof(k) / sizeof(char*)) {
        ...
        if (ret != PAPYRUSKV_OK) printf("FAILED [%s:%d] ret[%d] rank[%d] key[%s]\n", __FILE__, __LINE__, ret, rank, k[left]);
    ...
    if (left < sizeof(k) / sizeof(char*)) {
        ...
        if (ret == PAPYRUSKV_ERR && val == NULL && vallen == 0UL) printf("[%s:%d] GET:rank[%d] left[%d] key[%s] value[%s] vallen[%lu]\n", __FILE__, __LINE__, rank, left, k[left], val, vallen);
        else printf("[%s:%d] FAILED:rank[%d] left[%d] key[%s] value[%s] vallen[%lu]\n", __FILE__, __LINE__, rank, left, k[left], val, vallen);
    }
