...
int main(int argc, char** argv) {
    ...
    if (ret != PAPYRUSKV_OK) printf("[%s:%d] FAILED:ret[%d]\n", __FILE__, __LINE__, ret);
    printf("[%s:%d] PROECT:rank[%d] prot[%x]\n", __FILE__, __LINE__, rank, prot);
    ...
    if (left < sizeof(k) / sizeof(char*)) {
        ...
        else printf("[%s:%d] FAILED:rank[%d] left[%d] key[%s]\n", __FILE__, __LINE__, rank, left, k[left]);
    }
    ...
    if (rank < sizeof(k) / sizeof(char*)) {
        ...
        if (val == NULL && vallen == 0UL) printf("[%s:%d] FAILED:rank[%d] key[%s] value[%s] vallen[%lu]\n", __FILE__, __LINE__, rank, k[rank], val, vallen);
        else printf("[%s:%d] rank[%d] key[%s] value[%s] vallen[%lu]\n", __FILE__, __LINE__, rank, k[rank], val, vallen);
    }
