...
/*--------------------------------------------------------------------------
   ...
   Sample runs:  mpirun -np 4 hypre-test
                 mpirun -np 4 hypre-test -P 2 2 1 -n 50 50 100
                 mpirun -np 8 hypre-test -P 2 2 2 -n 50 50 50
                 mpirun -np <px*py*pz> hypre-test -P <px> <py> <pz> -n <nx> <ny> <nz>
		 hypre-test -help   for information
		 `mpirun -np' or `srun -n' depending on machine environment
   Description:  This example solves the 3-D Laplacian problem with zero boundary
                 conditions on an nx*px x ny*py x nz*pz grid using AMG-PCG.  
		 The number of unknowns is nx*ny*nz*px*py*pz.
                 The standard 7-point stencil is used, and we solve for the
                 interior nodes only.
...
HYPRE_BigInt hypre_map27( HYPRE_BigInt  ix, HYPRE_BigInt  iy, HYPRE_BigInt  iz, HYPRE_Int  px, HYPRE_Int  py,
             HYPRE_Int  pz, HYPRE_BigInt  Cx, HYPRE_BigInt  Cy, HYPRE_BigInt  Cz, HYPRE_Int nx, HYPRE_Int nxy);
      ...
      char *argv[] )
{
   ...
   /*-----------------------------------------------------------
    * Problem: Solve a 7pt 3D Laplace problem with AMG-PCG
    *-----------------------------------------------------------*/
   ...
   hypre_MPI_Barrier(comm);
   hypre_EndTiming(time_index);
   hypre_GetTiming("AMG Setup Time", &wall_time, comm);
   hypre_FinalizeTiming(time_index);
   hypre_ClearTiming();
   fflush(NULL);
   ...
   hypre_MPI_Barrier(comm);
   hypre_EndTiming(time_index);
   hypre_GetTiming("AMG-PCG Solve Time", &wall_time, comm);
   hypre_FinalizeTiming(time_index);
   hypre_ClearTiming();
   fflush(NULL);
